...
%Carrying Capacities
if feeding == 0
    K = 5;     %Estonia = 2, Spain = 5
elseif feeding == 1
    K = 10;     %Estonia = 4, Spain = 10
end
...
if feeding == 0
    b_1 = 1/1.388*((avga - b_D)*alpha - b_0*(b_D))/(b_D*K);
elseif feeding == 1
    b_1 = 1/1.086*((avga - b_D)*alpha - b_0*(b_D))/(b_D*K);
end
...
%E - environmental, F - frequency dependent

beta_F = 63;
beta_E = 2;   

%In Spain no over-wintered carcasses but quicker degradation
d = 52/dw;       %Base Line 52/1 for Spain
