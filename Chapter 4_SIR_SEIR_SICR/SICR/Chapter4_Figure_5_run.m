...
cvec = 0:0.05:1;
%% Alpha = 80, Epsilon = 0
load('SICRcEps0Alpha80it100t500')
...
%% Alpha = 40, Epsilon = 0
load('SICRcEps0Alpha40it100t500')
sz = size(minorextinct);
avgmin = zeros(1, sz(2)); 
cimin = zeros(1, sz(2));
for i = 1:sz(2)
...
load('SICRcEps05Alpha80it100t500')
sz = size(minorextinct);
avgmin = zeros(1, sz(2)); 
cimin = zeros(1, sz(2));
for i = 1:sz(2)
...
xlabel('c');
set(gca, 'Position', [0.52 0.1 0.21 0.405])
str = 'B(ii)';
dim = [.69 .465 .03 .03];
...
load('SICRcEps05Alpha40it100t500')
sz = size(minorextinct);
avgmin = zeros(1, sz(2)); 
cimin = zeros(1, sz(2));
for i = 1:sz(2)
...
xticks([0 0.2 0.4 0.6 0.8 1])
xticklabels([])
set(gca, 'Position', [0.52 0.55 0.21 0.405])
str = 'B(i)';
dim = [.69 .915 .03 .03];
annotation('textbox', dim, 'String', str, 'FitBoxToText', 'off', 'EdgeColor', 'w');
hold on